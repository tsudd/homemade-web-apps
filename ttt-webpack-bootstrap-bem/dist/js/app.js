...
let info = document.getElementsByClassName('turn-header');
let player = document.getElementsByClassName('turn');
$(player).addClass(ticClass);
let elmnts = document.getElementsByClassName('table__cell');
$(info).html('Turn');
...
let restart = function restartGame() {
    ...
    $(info).html('Turn');
    $(player).addClass(ticClass);
    let elements = document.getElementsByClassName('table__cell');
    for (let i = 0, k = 0; i < 3; i++) {
        for (let j = 0; j < 3; j++, k++) {
            cells[i][j].unfree = false;
            cells[i][j].content = 'o';
            $(elements[k]).removeClass([ticClass, tacClass]);
        }   
    }
}
